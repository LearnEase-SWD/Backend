...
namespace LearnEase.Service.Services
{
	public class CourseService : ICourseService
	{
		...
		public async Task<BaseResponse<IEnumerable<CourseResponse>>> GetAvailableCoursesAsync(int pageIndex, int pageSize)
		{
			if (pageIndex < 1) pageIndex = 1;
			if (pageSize < 1) pageSize = 10;
			...
			try
			{
				...
				// 1. Eager-load Topic để giảm số lần truy vấn
				var query = courseRepository.Entities.Include(c => c.Topic)
													 .Where(c => c.Status == CourseStatusConstant.AVAILABLE); // Chỉ lấy khóa học khả dụng

				// 2. Phân trang danh sách khóa học
				var paginatedResult = await courseRepository.GetPaggingAsync(query, pageIndex, pageSize);

				// 3. Lấy danh sách CourseId
				var courseIds = paginatedResult.Items.Select(c => c.CourseID).ToList();

				var lessons = new List<Lesson>();

				// 4. Lấy dữ liệu bài học theo thứ tự (tránh lỗi DbContext)
				foreach (var courseId in courseIds)
				{
					var lessonRepository = _unitOfWork.GetCustomRepository<ILessonRepository>();
					var courseLessons = await lessonRepository.GetLessonsByCourseId(courseId, pageIndex, pageSize);
					lessons.AddRange(courseLessons.Items);
				}

				// 5. Ánh xạ CourseResponse
				var courseResponses = paginatedResult.Items
					.Where(course => course.Status == CourseStatusConstant.AVAILABLE)
					.Select(course =>
					{
						var courseResponse = _mapper.Map<CourseResponse>(course);
						courseResponse.TopicName = course.Topic.Name;
						...
						courseResponse.Lessons = _mapper.Map<IEnumerable<LessonResponse>>(courseLessons);
	...
	.ToListAsync();
		...
		public async Task<BaseResponse<bool>> DeleteCourseAsync(Guid id)
		{
			...
			try
			{
				...
				existingCourse.Status = CourseStatusConstant.NOAVAILABLE;
				await courseRepository.UpdateAsync(existingCourse);
				await _unitOfWork.SaveAsync();
				await _unitOfWork.CommitTransactionAsync();
